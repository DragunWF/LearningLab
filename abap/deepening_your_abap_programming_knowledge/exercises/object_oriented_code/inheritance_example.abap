...
CLASS z8141_inheritance DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS z8141_inheritance IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA cargo_plane TYPE REF TO lcl_cargo_plane.
    CREATE OBJECT cargo_plane
      EXPORTING
        iv_manufacturer = 'DragunWF'
        iv_type         = 'Cargo'
        iv_cargo        = 10000000.

    DATA passenger_plane TYPE REF TO lcl_passenger_plane.
    CREATE OBJECT passenger_plane
      EXPORTING
        iv_manufacturer = 'Archaon'
        iv_type         = 'Passenger'
        iv_seats        = 30.

    out->write( 'Cargo Plane' ).
    out->write( cargo_plane->get_attributes(  ) ).

    out->write( '-------------------------------' ).

    out->write( 'Passenger Plane' ).
    out->write( passenger_plane->get_attributes(  ) ).
