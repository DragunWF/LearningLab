...
CLASS z8141_sorted_and_hashed_tables DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS z8141_sorted_and_hashed_tables IMPLEMENTATION.
...
CLASS lcl_flights DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    DATA standard_table TYPE STANDARD TABLE OF z8141_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
    DATA sorted_table TYPE SORTED TABLE OF z8141_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
    DATA hashed_table TYPE HASHED TABLE OF z8141_flights WITH UNIQUE KEY carrier_id connection_id flight_date.
...
CLASS lcl_flights IMPLEMENTATION.
  ...
  METHOD constructor.
    SELECT FROM z8141_flights FIELDS * INTO TABLE @standard_table.
    SELECT FROM z8141_flights FIELDS * INTO TABLE @sorted_table.
    SELECT FROM z8141_flights FIELDS * INTO TABLE @hashed_table.
