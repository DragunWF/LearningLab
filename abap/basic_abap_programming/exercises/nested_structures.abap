CLASS zcl_392_nested_structures DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_392_nested_structures IMPLEMENTATION.
...
* Example 5: My own example to practice
**********************************************************************
    out->write(  `-------------------------------------------------------------` ).
    out->write(  `Example 5: My own example to practice` ).

    TYPES: BEGIN OF schoolOfMagic,
             element    TYPE string,
             powerLevel TYPE i,
           END OF schoolOfMagic.

    TYPES: BEGIN OF wizard,
             firstName         TYPE string,
             lastName          TYPE string,
             yearsOfExperience TYPE i,
             school            TYPE schoolOfMagic,
           END OF wizard.

    DATA fireMagic TYPE schoolOfMagic.
    DATA dragunwf TYPE wizard.
    ...
    out->write( dragunwf ).
    out->write( `-------------------------------------------------------------` ).
    out->write( |School of Magic: Element ({ dragunwf-school-element }) ... Power level ({ dragunwf-school-powerLevel })| ).
