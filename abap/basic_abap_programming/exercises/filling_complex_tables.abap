CLASS zcl_392_filling_complex_tables DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_392_filling_complex_tables IMPLEMENTATION.
...
**********************************************************************
    ...
    TYPES: BEGIN OF user,
           ...
           password TYPE string,
           END OF user.
    TYPES tt_user TYPE STANDARD TABLE OF user
                  WITH UNIQUE KEY username.
    ...
    out->write(  `--------------------------------------------` ).
    out->write(  `Example 5: My own example for practice` ).
    out->write( accounts ).
    out->write( `.` ).
    out->write( users ).
