CLASS zcl_392_complex_tables_access DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_392_complex_tables_access IMPLEMENTATION.
...
* Example 6: My own example for practice
**********************************************************************
    ...
    TYPES: BEGIN OF account,
             ...
             email    TYPE string,
           END OF account.
    TYPES tt_account TYPE STANDARD TABLE OF account
                     WITH NON-UNIQUE KEY username.
    DATA accounts TYPE tt_account.

    TYPES: BEGIN OF user,
             username TYPE string,
             password TYPE string,
           END OF user.
    TYPES tt_user TYPE SORTED TABLE OF user
                  WITH UNIQUE KEY username.
    DATA users TYPE tt_user.

    accounts = VALUE #(
        ( username = 'DragunWF' password = 'password123' email = 'test@gmail.com' )
        ( username = 'CPTInvincible' password = 'reader123' email = 'cpt231432@gmail.com')
        ( username = 'Luq' password = 'deltarune123' email = 'luq231321@gmail.com')
    ).
    users = CORRESPONDING #( accounts ).

    DATA logged_in_user TYPE user.
    logged_in_user = users[ username = 'DragunWF' ].
    logged_in_user-password = 'newpassword'.

    out->write(  `----------------------------` ).
    out->write(  `Example 6: My own example for practice` ).
    out->write( accounts ).
    out->write( `.` ).
    out->write( users ).
    out->write( `.` ).
    out->write( logged_in_user ).
