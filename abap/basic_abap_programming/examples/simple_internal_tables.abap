...
* Example 6: My own example
**********************************************************************
    out->write(  `-----------------------------` ).
    out->write(  `Example 6: My own example for practice` ).
    out->write(  `-----------------------------` ).

    DATA: dragunNumbers TYPE TABLE OF i,
          iterations TYPE i VALUE 10,
          count TYPE i VALUE 1.

    DO iterations TIMES.
        APPEND SY-INDEX + 3 * 2 TO dragunNumbers.
    ENDDO.

    LOOP AT dragunNumbers INTO DATA(element).
        out->write( |Element { count }: { element }| ).
        count += 1.
    ENDLOOP.
