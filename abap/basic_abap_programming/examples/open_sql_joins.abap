CLASS zcl_392_joins DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_392_joins IMPLEMENTATION.
...
* Example 7: My own example for practice
**********************************************************************

    DATA dragun_connection TYPE st_connection.
    DATA dragun_connection_short TYPE st_connection_short.
    DATA dragun_connection_full TYPE /DMO/I_Connection.

    SELECT SINGLE
        FROM ( /dmo/connection  AS a
        LEFT OUTER JOIN /dmo/airport AS b
            ON a~airport_from_id = b~airport_id )
        LEFT OUTER JOIN /dmo/airport AS c
            ON a~airport_to_id = c~airport_id
     FIELDS a~airport_from_id, a~airport_to_id,
            b~name AS airport_from_name, c~name AS airport_to_name
        WHERE a~carrier_id    = 'LH'
          AND a~connection_id = '0400'
         INTO @DATA(dragun_connection_join).
    out->write(  `------------------------------------------` ).
    out->write(  `Example 7: My own example for practice` ).
    out->write( dragun_connection_join ).
    out->write( dragun_connection ).
    out->write( dragun_connection_short ).
    out->write( dragun_connection_full ).
