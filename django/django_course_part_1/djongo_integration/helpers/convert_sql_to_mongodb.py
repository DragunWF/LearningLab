...
def smart_split(row_str):
    """
    Splits a row string into individual values,
    respecting commas inside quoted strings.
    """
    lexer = shlex.shlex(row_str, posix=True)
    lexer.whitespace = ','
...
def sql_insert_to_mongodb(sql_query):
    ...
    for row_match in row_pattern.finditer(values_text):
        ...
        row_values = []
        for val in smart_split(raw_row):
            val = val.strip()
            # Handle null values
            if val.lower() == 'null':
